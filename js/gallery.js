...
function onKeydowmSibling(evt) {
  const currentEl = gallerysItems.findIndex(
    ({ original }) => original === refs.image.src,
  );
  if (evt.code === 'ArrowRight') {
    refs.image.src = `${
      gallerysItems[(currentEl + 1) % gallerysItems.length].original
    }`;
  } else if (evt.code === 'ArrowLeft') {
    if (currentEl === 0) {
      refs.image.src = `${
        gallerysItems[
          (currentEl + gallerysItems.length - 1) % gallerysItems.length
        ].original
      }`;
    } else {
      refs.image.src = `${
